...
def main():
    ...
    p.add_argument('--field', default='Data/mixingRatio/XCO2', help='GOSAT field, e.g. Data/mixingRatio/XCO2BiasCorrected')
    ...
    p.add_argument('--product-scale', type=float, default=1.0, help='Multiplier converting TIFF values to ppm')
    p.add_argument('--reference-scale', type=float, default=1.0, help='Multiplier converting GOSAT ppm to ppm')
    p.add_argument('--min-coverage', type=float, default=0.0, help='Minimum valid TIFF area / full GOSAT cell area (0..1)')
    p.add_argument('--mask-value', type=int, default=None, help='Optional accepted GOSAT XCO2Mask value; unset retains valid field values')
    ...
    tif = args.tiff or BASE / 'evidence/workflow_outputs' / f'IRIDE-S_S5-02-08_{stamp}_V1.tif'
    ref = args.reference or BASE / 'ground_truth/gosat_fts_l3_co2_v03_05' / args.month / f'SWIRL3CO2_{stamp}_V03.05.tar'
    ...
    with tarfile.open(ref) as archive:
        ...
        with h5py.File(io.BytesIO(archive.extractfile(member).read())) as h:
            ...
            mask = h['Data/maskInformation/XCO2Mask'][:]
    ...
    for i, j in np.ndindex(values.shape):
        ...
        rows.append(dict(month=args.month, latitude=float(lat[i,j]), longitude=float(lon[i,j]), product_ppm=pred, reference_ppm=truth, difference_ppm=pred-truth, coverage_fraction=float(coverage), valid_pixels=int(good.sum()), gosat_mask=int(mask[i,j])))
    if not rows:
        raise ValueError('No valid matched cells under selected settings')
    truth = np.array([r['reference_ppm'] for r in rows])
    errors = np.array([r['difference_ppm'] for r in rows])
